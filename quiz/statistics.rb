...
module Quiz
  ...
  class Statistics
    ...
    def record_answer(question, answer, user_answer, is_correct)
      ...
      # 上限を超える場合は古い記録を削除
      @records = @records.last(Constants::RECORDS_LIMIT) if @records.length > Constants::RECORDS_LIMIT
    ...
    def session_completed?
      @current_session.length >= Constants::SESSION_SIZE
    end

    # 現在のセッションを完了として記録
    def complete_current_session
      return if @current_session.length != Constants::SESSION_SIZE
      ...
      # 最新セッションのみ保持
      if @sessions.length > Constants::DISPLAY_SESSIONS_LIMIT
        @sessions = @sessions.last(Constants::DISPLAY_SESSIONS_LIMIT)
      end
    ...
    # 直近のセッション記録を取得
    def recent_sessions(count = Constants::DISPLAY_SESSIONS_LIMIT)
